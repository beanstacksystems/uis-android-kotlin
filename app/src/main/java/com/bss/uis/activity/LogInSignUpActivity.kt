...
class LogInSignUpActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!ContextPreferenceManager().isUserLogedOut(this@LogInSignUpActivity)) {
            mainScope.launch {
               ...
               startActivity(Intent(this@LogInSignUpActivity,EnableBiometricLoginActivity::class.java))
    ...
    private fun initView() {
        ...
        fbLoginButton.setPermissions(fbPermission)
        fbLoginButton = findViewById(R.id.fb_login_button)
...
//            fbLoginButton.performClick()
    ...
    private suspend fun updateLocalDB() {
        val applicationDao = UISDatabase.getInstance(this).applicationDao
...
//        initAppConfig(applicationRepository.allWords)
    }
        ...
        viewModelUIS.masterDataResponseList.observe(this) {
            when (it) {
                ...
                is Resource.Success -> {
                    ...
                    GlobalScope.launch { it.data?.let { it1 -> saveToMasterEntity(it1) } }
        ...
        viewModelUIS.userApiResponse.observe(this) {
            when (it) {
                ...
                is Resource.Success -> {
                    ...
                    GlobalScope.launch { pullUserRightData(ContextPreferenceManager().getToken("token", this@LogInSignUpActivity)
                        .toString(), roleidlist) }
        ...
        viewModelUIS.userrightList.observe(this) {
            when (it) {
                ...
                is Resource.Success -> {
                    ...
                    GlobalScope.launch {
                        it.data?.let { it1 -> saveUserRights(it1) }
                    }
...
//                startBiometric()
    ...
    private suspend fun saveToMasterEntity(masterValueDTOList: List<MasterDataResponseDomain>) {
        ...
        for (masterValueDTO in masterValueDTOList) {
            val masterData = MasterData()
            masterData.masterdatadesc = masterValueDTO.masterdatadesc
            masterData.masterdataId = masterValueDTO.masterdataId!!
            masterData.masterdataType = masterValueDTO.masterdataType
            masterData.masterdataval = masterValueDTO.masterdataval
            masterData.isactive = masterValueDTO.isactive
        ...
        mainScope.launch {
            masterDAORepository.masterDataList.observe(this@LogInSignUpActivity) {
                it.forEach { data ->
                    Log.d("masterDAORepository", data.masterdataval.toString())
...
//                handleAccessToken(
//                    account.idToken, account.serverAuthCode as Nothing?,
//                    account.email!!, account.id!!, account.displayName!!, "google"
//                )
